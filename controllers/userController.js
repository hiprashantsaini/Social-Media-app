...
const loginUser = async (req, res) => {
  try {
    ...
    if (existingDevice) {
      ...
      res.status(200).send({ status: true, deviceOtp: false, message: "Login successfull", token: token, user: updatedUser });
    } else {
...
const loginWithGoogle = async (req, res) => {
  try {
    ...
    const userInfo = await axios.get(`https://www.googleapis.com/oauth2/v1/userinfo?access_token=${data.access_token}`)
    const user = userInfo.data;

    const deviceInfo = findDeviceInfo(req);

    const checkUser = await User.findOne({ email: user.email }).populate('subscription.planId');
    if (!checkUser) {
      ...
      const token = jwt.sign({ userId: udata._id }, process.env.JWT_SECRET, { expiresIn: '5h' });
      res.status(200).send({ status: true, deviceOtp: false, message: "Login successfull", token: token, user: udata });
    } else {
      ...
      const token = jwt.sign({ userId: finalUserData._id }, process.env.JWT_SECRET, { expiresIn: '5h' });
      res.status(200).send({ status: true, deviceOtp: false, message: "Login successfull. User already exists", token: token, user: finalUserData });
    }
